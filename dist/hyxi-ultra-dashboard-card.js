class HyxiUltraDashboardCard extends HTMLElement {constructor() {super();this.attachShadow({ mode: "open" });}

setConfig(config) {this.config = {serial: "123456789",language: "en",capacity: 9.2,investment: 2222.99,electricity_price: 0.22568,feed_in_price: 0.10,
...
getEntityValue(entity, fallback = 0) {const state = this._hass?.states?.[entity]?.state;const value = parseFloat(state);return Number.isFinite(value) ? value : fallback;}

getEntities() {const serial = this.config.serial || "123456789";
...
getLabels() {const nl = this.config.language === "nl";
...
parseTimeToMinutes(value, fallback) {if (typeof value !== "string") return fallback;
...
isOffpeakNow() {const now = new Date();const day = now.getDay(); // 0 = Sunday, 6 = Saturdayconst weekendOffpeak = this.config.weekend_offpeak !== false;
...
getTariffInfo(labels) {const tariffMode = this.config.tariff_mode || "simple";
...
render() {if (!this._hass || !this.config) return;
...
getGridOptions() {return {columns: 12,rows: 4,min_columns: 4,min_rows: 3,};}

getCardSize() {return 6;}

static getStubConfig() {return {serial: "123456789",capacity: 9.2,investment: 2222.99,electricity_price: 0.22568,feed_in_price: 0.10,tariff_mode: "simple",normal_electricity_price: null,offpeak_electricity_price: null,feed_in_compensation: null,feed_in_costs: 0,offpeak_start: "23:00",offpeak_end: "07:00",weekend_offpeak: true,extra_benefit: 33.38,start_date: "2026-05-01",language: "en",image: "/hacsfiles/HYXi-Ultra-Dashboard/hyxihalopgnnieuw.png",charged_correction: 0,discharged_correction: 0,};}}

if (!customElements.get("hyxi-ultra-dashboard-card")) {customElements.define("hyxi-ultra-dashboard-card", HyxiUltraDashboardCard);}

window.customCards = window.customCards || [];window.customCards.push({type: "hyxi-ultra-dashboard-card",name: "HYXi Ultra Dashboard Card",description: "A neon dashboard card for HYXi HALO battery systems with bundled image, dynamic LED ring, SOC overlay and optional Dutch dual tariff mode.",});
