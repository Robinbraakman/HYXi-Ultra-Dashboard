class HyxiUltraDashboardCard extends HTMLElement {
  ...
  setConfig(config) {
    this.config = {
      ...
      // nl_dual = current savings uses active tariff, total savings uses weighted average tariff from start_date
      tariff_mode: "simple",
  ...
  getLabels() {
    ...
    return {
      ...
      totalTariffAverage: nl ? "Gem. tarief totaal" : "Total average tariff",
  ...
  isOffpeakNow() {
    return this.isOffpeakAt(new Date());
  }

  getDualTariffSettings() {
    const fallbackElectricityPrice = this.getNumber(this.config.electricity_price, 0);
    const fallbackFeedInPrice = this.getNumber(this.config.feed_in_price, 0);

    const normalPrice = this.getNumber(
      this.config.normal_electricity_price !== null && this.config.normal_electricity_price !== undefined
        ...
        : fallbackElectricityPrice,
      fallbackElectricityPrice
    );

    const offpeakPrice = this.getNumber(
    ...
    const feedInCompensation = this.getNumber(
    ...
    const feedInCosts = this.getNumber(this.config.feed_in_costs, 0);
  ...
  getTariffInfo(labels) {
    ...
    if (tariffMode === "nl_dual") {
      const settings = this.getDualTariffSettings();
      const isOffpeak = this.isOffpeakNow();
      const electricityPrice = isOffpeak ? settings.offpeakPrice : settings.normalPrice;
      const benefit = electricityPrice - settings.feedInPrice;

      return {
        mode: "nl_dual",
        label: isOffpeak ? labels.offpeakTariff : labels.normalTariff,
        electricityPrice,
        feedInPrice: settings.feedInPrice,
        benefit,
      };
    }

    const electricityPrice = this.getNumber(this.config.electricity_price, 0);
    const feedInPrice = this.getNumber(this.config.feed_in_price, 0);
  ...
  getTotalTariffInfo(labels) {
    const tariffMode = this.config.tariff_mode || "simple";

    if (tariffMode !== "nl_dual") {
      return this.getTariffInfo(labels);
    }

    const settings = this.getDualTariffSettings();
    const currentTariff = this.getTariffInfo(labels);

    // Safe weighted estimate without browser-heavy history tracking.
    // This keeps the card simple: no helpers, no automations and no extra sensors needed.
    // Total savings uses an estimated normal/off-peak distribution based on the configured hours.
    const start = new Date(`${this.config.start_date || "2026-05-01"}T00:00:00`);
    const now = new Date();
    ...
    if (Number.isNaN(start.getTime()) || start >= now) {
      return {
        ...
        normalShare: currentTariff.label === labels.normalTariff ? 100 : 0,
        offpeakShare: currentTariff.label === labels.offpeakTariff ? 100 : 0,
      };
    }

    const startMinutes = this.parseTimeToMinutes(this.config.offpeak_start, 23 * 60);
    const endMinutes = this.parseTimeToMinutes(this.config.offpeak_end, 7 * 60);
    const minutesPerDay = 24 * 60;

    let offpeakMinutesPerWeekday = 0;

    if (startMinutes === endMinutes) {
      offpeakMinutesPerWeekday = 0;
    } else if (startMinutes < endMinutes) {
      offpeakMinutesPerWeekday = endMinutes - startMinutes;
    } else {
      offpeakMinutesPerWeekday = (minutesPerDay - startMinutes) + endMinutes;
    }
    ...
    while (cursor < now && guard < maxDays) {
      const nextDay = new Date(cursor);
      nextDay.setDate(nextDay.getDate() + 1);

      const periodStart = cursor < start ? start : cursor;
      const periodEnd = nextDay < now ? nextDay : now;
      const dayMs = Math.max(0, periodEnd - periodStart);

      const day = cursor.getDay(); // 0 = Sunday, 6 = Saturday
      const isWeekend = day === 0 || day === 6;

      if (this.config.weekend_offpeak !== false && isWeekend) {
        offpeakMs += dayMs;
      } else {
        const offpeakShare = Math.max(0, Math.min(offpeakMinutesPerWeekday / minutesPerDay, 1));
        offpeakMs += dayMs * offpeakShare;
        normalMs += dayMs * (1 - offpeakShare);
      }
    ...
    const totalMs = normalMs + offpeakMs;

    if (totalMs <= 0) {
      return currentTariff;
    }

    const averageElectricityPrice =
      ((normalMs * settings.normalPrice) + (offpeakMs * settings.offpeakPrice)) / totalMs;

    const benefit = averageElectricityPrice - settings.feedInPrice;

    return {
      mode: "nl_dual_weighted",
      label: labels.totalTariffAverage,
      electricityPrice: Number.isFinite(averageElectricityPrice) ? averageElectricityPrice : currentTariff.electricityPrice,
      feedInPrice: settings.feedInPrice,
      benefit: Number.isFinite(benefit) ? benefit : currentTariff.benefit,
      isWeightedAverage: true,
      normalShare: totalMs > 0 ? (normalMs / totalMs) * 100 : 0,
      offpeakShare: totalMs > 0 ? (offpeakMs / totalMs) * 100 : 0,
    };
  }
  ...
  render() {
    ...
    const currentTariffInfo = this.getTariffInfo(labels);
    const totalTariffInfo = this.getTotalTariffInfo(labels);

    const totalSavings = discharged * totalTariffInfo.benefit;
    const currentSavings = (dischargingPower / 1000) * currentTariffInfo.benefit;
...
window.customCards.push({
  ...
  description: "A neon dashboard card for HYXi HALO battery systems with bundled image, dynamic LED ring, SOC overlay and optional Dutch dual tariff mode with weighted average total savings.",
});
